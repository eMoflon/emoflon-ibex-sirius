...
public class DAnalysisSessionServicesImpl implements SessionService, DAnalysisSessionService {
	 ...
	 *            the session.
	 */
	public DAnalysisSessionServicesImpl(CustomDAnalysisSessionImpl session) {
	...
	private void clearRepresentationData(final EObject associatedInstance,
			final Collection<DAnalysis> analysisAndReferenced) {
	...
	private void clearGMFDiagramsData(final EObject associatedInstance, final Collection<Resource> resources,
			Collection<DAnalysis> analysisAndReferenced) {
	...
	private Collection<EObject> getRepresentationData(final EObject associatedInstance,
			final Collection<DAnalysis> analysisAndReferenced) {
	...
	private Collection<EObject> getRepresentationFromDescData(final EObject associatedInstance,
			Collection<DAnalysis> analysisAndReferenced) {
		final Collection<EObject> datas = new ArrayList<>();
		for (DAnalysis analysis : analysisAndReferenced) {
			for (final DView view : analysis.getOwnedViews()) {
				...
				while (it.hasNext()) {
					final DRepresentation rep = it.next();
					final RepresentationDescription currentRepresentationDescription = DialectManager.INSTANCE
							.getDescription(rep);
	...
	private Collection<EObject> getGMFDiagramsData(final EObject representation) {
		...
		if (representation instanceof DRepresentation) {
			...
			for (AnnotationEntry annotation : new DRepresentationQuery(dRepresentation)
	...
	private Collection<EObject> getFeatureExtensionsData(final EObject associatedInstance,
			Collection<Resource> resources) {
		final Collection<EObject> datas = new ArrayList<>();
		Collection<Resource> allResources = new ArrayList<>();
		allResources.addAll(resources);
		// We also need to looking for the data in the given associatedInstance
		// resource. (srm file for instance)
		if (associatedInstance != null) {
	...
	public void putCustomData(final String key, final EObject associatedInstance, final EObject data) {
		if (CustomDataConstants.GMF_DIAGRAMS.equals(key)) {
			if (associatedInstance instanceof DRepresentation) {
				SiriusHelper.getOrCreateAnnotation(CustomDataConstants.GMF_DIAGRAMS,
						(DRepresentation) associatedInstance, data);
			}
	 ...
	 * associated {@link DRepresentation} as root object of the {@link DAnalysis}
	 * resource.
	 * 
	...
	private void addRepresentationToContainer(final DRepresentation representation, final Resource semanticResource) {
		...
		DView dView = DAnalysisSessionHelper.findContainerForAddedRepresentation(semanticRoot, viewpoint,
				session.allAnalyses(), analysisSelector, representation);

		if (dView == null) {
			dView = DAnalysisSessionHelper.findFreeContainerForAddedRepresentation(viewpoint, semanticRoot,
					session.getAnalyses(), analysisSelector, representation);
			if (dView != null) {
		...
		if (dView == null) {
			...
			dView.setViewpoint(viewpoint);
			final DAnalysis analysis = DAnalysisSessionHelper.selectAnalysis(viewpoint, session.allAnalyses(),
					analysisSelector, representation);
			analysis.getOwnedViews().add(dView);
		}

		Resource resourceforRepresentation = representationLocationManager
				.getOrCreateRepresentationResource(representation, dView.eResource());
		...
		final DRepresentationDescriptor descriptor = DRepresentationDescriptorInternalHelper
				.createDescriptor(representation);
